(function (global, factory) {
  ...
  var FUNCTION_THREW_ERROR = function FUNCTION_THREW_ERROR(fnName, type) {
    return "Exception thrown when executing ".concat(type, " ").concat(fnName || "");
  };
  var INVALID_ACTION_FACTORY_EXECUTED = function INVALID_ACTION_FACTORY_EXECUTED(actionName, type) {
    return "".concat(FUNCTION_THREW_ERROR(actionName, type), "\nThe ").concat(type, " returned a value which is not an action.");
  };
  var INVALID_PREDICATE_EXECUTED = function INVALID_PREDICATE_EXECUTED(actionName, type) {
    return "".concat(FUNCTION_THREW_ERROR(actionName, type), "\nThe ").concat(type, " returned a value which is not a boolean.");
  };
  var ACTION_FACTORY_DESC = "action factory";
  var ENTRY_ACTION_FACTORY_DESC = "(decorating) entry action";
  var UPDATE_STATE_FN_DESC = "update state function";
  var PREDICATE_DESC = "predicate";
  ...
  function make_states(stateList) {
    return stateList.reduce(function (acc, state) {
      acc[state] = "";
      return acc;
    }, {});
  }
  function make_events(eventList) {
    return eventList;
  }
  /**
  ...
  function mergeModelUpdates(arrayUpdateActions) {
    return function (extendedState, eventData, settings) {
      return {
        updates: arrayUpdateActions.reduce(function (acc, updateAction) {
          var update = updateAction(extendedState, eventData, settings).updates;
  ...
  function chainModelUpdates(arrayUpdateActions) {
    return function (extendedState, eventData, settings) {
      var updateState = settings.updateState;
      return {
        updates: arrayUpdateActions.reduce(function (acc, updateAction) {
          var extendedState = acc.extendedState,
              updates = acc.updates;
          var update = updateAction(extendedState, eventData, settings).updates;
          var updatedState = updateState(extendedState, updates);
          return {
            extendedState: updatedState,
            updates: update
          };
        }, {
  ...
  function mergeActionFactories(mergeOutputFn, arrayActionFactory) {
    return function (extendedState, eventData, settings) {
      var _ref3;

      var arrayActions = arrayActionFactory.map(function (factory) {
        return factory(extendedState, eventData, settings);
      });
      var arrayStateUpdates = arrayActions.map(function (x) {
        return x.updates || [];
      });
      var arrayOutputs = arrayActions.map(function (x) {
        return x.outputs || {};
      });
      return {
        updates: (_ref3 = []).concat.apply(_ref3, _toConsumableArray(arrayStateUpdates)),
  ...
  function identity(extendedState, eventData, settings) {
    return {
      updates: [],
      outputs: NO_OUTPUT
    };
  ...
  function formatActionName(action, from, event, to, predicate) {
    var predicateName = predicate ? predicate.name : "";
    var formattedPredicate = predicateName ? "[".concat(predicateName, "]") : "";
    var actionName = action ? action.name : "identity";
    var formattedAction = actionName ? actionName : "unnamed action";
    return "".concat(formattedAction, ":").concat(from, "-").concat(event, "->").concat(to, " ").concat(formattedPredicate);
  }

  function getFsmStateList(states) {
    var getLabel = objectTreeLenses.getLabel;
    var traverse = {
      strategy: PRE_ORDER,
      seed: {},
      visit: function visit(accStateList, traversalState, tree) {
    ...
    var stateHashMap = traverseObj(traverse, states);
    return stateHashMap;
  }
  ...
  function mapOverTransitionsActions(mapFn, transitions) {
    return reduceTransitions(function (acc, transition, guardIndex, transitionIndex) {
      ...
      var mappedAction = mapFn(action, transition, guardIndex, transitionIndex);
  ...
  function everyTransition(pred, transition) {
    return reduceTransitions(function (acc, flatTransition) {
      return acc && pred(flatTransition);
    }, true, [transition]);
  }
  function computeTimesCircledOn(edgePath, edge) {
    return edgePath.reduce(function (acc, edgeInEdgePath) {
      return edgeInEdgePath === edge ? acc + 1 : acc;
    }, 0);
  }
  function isInitState(s) {
    return s === INIT_STATE;
  }
  function isInitEvent(e) {
    return e === INIT_EVENT;
  }
  function isEventless(e) {
  ...
  function getHistoryParentState(to) {
    return to[SHALLOW] || to[DEEP];
  }
  function isShallowHistory(to) {
    return to[SHALLOW];
  }
  function isDeepHistory(to) {
    return to[DEEP];
  }
  ...
  function isHistoryStateEdge(edge) {
  ...
  function isCompoundState(analyzedStates, controlState) {
    var statesAdjacencyList = analyzedStates.statesAdjacencyList;
    return statesAdjacencyList[controlState] && statesAdjacencyList[controlState].length !== 0;
  }
  function isAtomicState(analyzedStates, controlState) {
  ...
  function updateHistory(history, stateAncestors, state_from_name) {
  ...
  function computeHistoryState(states, controlStateSequence, historyType, historyParentState) {
    // NOTE : we compute the whole story every time. This is inefficient, but for now sufficient
    var _computeHistoryMaps = computeHistoryMaps(states),
    ...
    history = controlStateSequence.reduce(function (history, controlState) {
      return updateHistory(history, stateAncestors, controlState);
    }, history);
  ...
  function tryCatchMachineFn(fnType, fn) {
    var argsDesc = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
    return tryCatch(fn, function (e, args) {
      var err = new Error(e);
      var fnName = getFunctionName(fn); // NOTE : we concatenate causes but not `info`

      var probableCause = FUNCTION_THREW_ERROR(fnName, fnType);
      err.probableCause = e.probableCause ? [e.probableCause, probableCause].join('\n') : probableCause;
      var info = {
        fnName: fnName,
        params: argsDesc.reduce(function (acc, argDesc, index) {
  ...
  function getFunctionName(actionFactory) {
  ...
  function assert(contract, arrayParams) {
    var contractName = contract.name || contract.name.displayName || "";
  ...
  function notifyThrows(console, error) {
    console.error(error);
    error.probableCause && console.error("Probable cause: ".concat(error.probableCause));
  ...
  function handleFnExecError(notify, execInfo, actionResultOrError, postCondition, throwFn, invalidResultFn) {
    var debug = notify.debug,
  ...
  function notifyAndRethrow(_ref5, actionResultOrError) {
    var debug = _ref5.debug,
        console = _ref5.console;
    notifyThrows(console, actionResultOrError);
    throw actionResultOrError;
  }
  function throwIfInvalidActionResult(_ref6, actionResultOrError, exec) {
    var debug = _ref6.debug,
        console = _ref6.console;
    var action = exec.action,
        extendedState = exec.extendedState,
        eventData = exec.eventData,
        settings = exec.settings;
    var actionName = getFunctionName(action);
    var error = new Error(INVALID_ACTION_FACTORY_EXECUTED(actionName, ACTION_FACTORY_DESC));
    error.info = {
      fnName: getFunctionName(action),
      params: {
        updatedExtendedState: extendedState,
        eventData: eventData,
        settings: settings
      },
  ...
  function throwIfInvalidGuardResult(_ref7, resultOrError, exec) {
    var debug = _ref7.debug,
        console = _ref7.console;
    var predName = getFunctionName(exec.predicate);
    var error = new Error(INVALID_PREDICATE_EXECUTED(predName, PREDICATE_DESC));
    error.info = {
  ...
  function throwIfInvalidEntryActionResult(_ref8, exitActionResultOrError, exec) {
    var debug = _ref8.debug,
        console = _ref8.console;
    var action = exec.action,
        extendedState = exec.extendedState,
        eventData = exec.eventData,
        settings = exec.settings;
    var actionName = getFunctionName(action);
    var error = new Error(INVALID_ACTION_FACTORY_EXECUTED(actionName, ENTRY_ACTION_FACTORY_DESC));
    error.info = {
      fnName: getFunctionName(action),
      params: {
        updatedExtendedState: extendedState,
  ...
  function formatUndefinedInJSON(obj) {
  ...
  function build_nested_state_structure(states) {
    var root_name = "State";
    var hash_states = {};
    var is_group_state = {}; // Add the starting state
  ...
  function createStateMachine(fsmDef, settings) {
    var control_states = fsmDef.states,
        events = fsmDef.events,
        initialExtendedState = fsmDef.initialExtendedState,
    ...
    var throwKinglyError = function throwKinglyError(obj) {
    ...
    var wrappedUpdateState = function wrappedUpdateState(extendedState, updates) {
      var fnName = userProvidedUpdateStateFn.name || userProvidedUpdateStateFn.displayName || "";
    ...
    var transitions = normalizeTransitions(fsmDef); // Create the nested hierarchy

    var hash_states_struct = build_nested_state_structure(control_states); // This will be the extended state object which will be updated by all actions and on which conditions
    ...
    var extendedState = initialExtendedState; // history maps

    var _computeHistoryMaps = computeHistoryMaps(control_states),
        stateList = _computeHistoryMaps.stateList,
        stateAncestors = _computeHistoryMaps.stateAncestors;

    var history = initHistoryDataStructure(stateList); // @type {Object<state_name,boolean>}, allows to know whether a state has a init transition defined

    var is_init_state = {}; // @type {Object<state_name,boolean>}, allows to know whether a state has an automatic transition defined
    // that would be init transitions + eventless transitions

    var is_auto_state = {}; // @type {Object<state_name,boolean>}, allows to know whether a state is a group of state or not
    ...
    function send_event(event_struct, isExternalEvent) {
      ...
      var _destructureEvent = destructureEvent(event_struct),
          eventName = _destructureEvent.eventName,
          eventData = _destructureEvent.eventData;

      var current_state = getCurrentControlState();
      console.debug("send event", event_struct); // Edge case : INIT_EVENT sent and the current state is not the initial state
      ...
      var outputs = process_event(hash_states_struct.hash_states, eventName, eventData, extendedState);
      return outputs;
    }

    function process_event(hash_states, event, event_data, extendedState) {
      var current_state = hash_states[INIT_STATE].current_state_name;
      ...
      if (event_handler) {
        ...
        var _event_handler = event_handler(extendedState, event_data, current_state),
            stop = _event_handler.stop,
            rawOutputs = _event_handler.outputs;
        ...
        var outputs = arrayizeOutput(rawOutputs); // we read it anew as the execution of the event handler may have changed it

        var new_current_state = hash_states[INIT_STATE].current_state_name; // Two cases here:
        ...
        if (is_auto_state[new_current_state] && new_current_state !== current_state) {
          ...
          var auto_event = is_init_state[new_current_state] ? INIT_EVENT : AUTO_EVENT;
          tracer({
            ...
            trace: {
              ...
              event: _defineProperty({}, auto_event, event_data),
              machineState: {
                cs: getCurrentControlState(),
                es: extendedState,
                hs: history
              }
            }
          });
          var nextOutputs = send_event(_defineProperty({}, auto_event, event_data), false);
          tracer({
        ...
        console.warn("There is no transition associated to the event |".concat(event, "| in state |").concat(current_state, "|!"));
        tracer({
          type: WARN_MSG,
          trace: {
            info: {
              received: _defineProperty({}, event, event_data)
            },
            message: "There is no transition associated to the event |".concat(event, "| in state |").concat(current_state, "|!"),
            machineState: {
              cs: current_state,
              es: extendedState,
              hs: history
    ...
    function enter_next_state(to, updatedExtendedState, hash_states) {
      var state_to;
      var state_to_name; // CASE : history state (H)

      if (isHistoryControlState(to)) {
        var history_type = to.deep ? DEEP : to.shallow ? SHALLOW : void 0;
        var history_target = to[history_type]; // Edge case : history state (H) && no history (i.e. first time state is entered), target state
    ...
    function start() {
      ...
      return send_event(_defineProperty({}, INIT_EVENT, initialExtendedState), true);
    }
    ...
    transitions.forEach(function (transition) {
      ...
      var from_proto = hash_states[from]; // CASE : automatic transitions : no events - likely a transient state with only conditions
        ...
        var condition_checking_fn = function (guard, settings) {
          ...
          var condition_checking_fn = function condition_checking_fn(extendedState_, event_data, current_state) {
            ...
            if (shouldTransitionBeTaken) {
              ...
              leave_state(from, extendedState_, hash_states); // Update the extendedState before entering the next state
              ...
              var next_state = enter_next_state(to, updates, hash_states);
    ...
    return function yyield(x) {
      try {
        var _destructureEvent2 = destructureEvent(x),
  ...
  function makeHistoryStates(states) {
    var stateList = Object.keys(getFsmStateList(states)); // used for referential equality comparison to discriminate history type

    return function (historyType, controlState) {
      var _ref3;
      ...
      return _ref3 = {}, _defineProperty(_ref3, historyType, controlState), _defineProperty(_ref3, "type", history_symbol), _ref3;
    };
  }
  ...
  exports.create_state_machine = create_state_machine;
  exports.createStateMachine = createStateMachine;
  exports.makeWebComponentFromFsm = makeWebComponentFromFsm;
  exports.mergeOutputsFn = mergeOutputsFn;
  exports.makeHistoryStates = makeHistoryStates;
  exports.historyState = historyState;
  exports.toPlantUml = toPlantUml;
  exports.toDagreVisualizerFormat = toDagreVisualizerFormat;
  exports.CONTRACT_MODEL_UPDATE_FN_RETURN_VALUE = CONTRACT_MODEL_UPDATE_FN_RETURN_VALUE;
  exports.SEP = SEP;
  exports.TRANSITION_SYMBOL = TRANSITION_SYMBOL;
  exports.TRANSITION_LABEL_START_SYMBOL = TRANSITION_LABEL_START_SYMBOL;
  exports.HISTORY_STATE_NAME = HISTORY_STATE_NAME;
  exports.HISTORY_PREFIX = HISTORY_PREFIX;
  exports.INIT_STATE = INIT_STATE;
  exports.INIT_EVENT = INIT_EVENT;
  exports.AUTO_EVENT = AUTO_EVENT;
  exports.STATE_PROTOTYPE_NAME = STATE_PROTOTYPE_NAME;
  exports.NO_STATE_UPDATE = NO_STATE_UPDATE;
  exports.NO_OUTPUT = NO_OUTPUT;
  exports.ACTION_IDENTITY = ACTION_IDENTITY;
  exports.history_symbol = history_symbol;
  exports.SHALLOW = SHALLOW;
  exports.DEEP = DEEP;
  exports.WRONG_EVENT_FORMAT_ERROR = WRONG_EVENT_FORMAT_ERROR;
  exports.FUNCTION_THREW_ERROR = FUNCTION_THREW_ERROR;
  exports.INVALID_ACTION_FACTORY_EXECUTED = INVALID_ACTION_FACTORY_EXECUTED;
  exports.INVALID_PREDICATE_EXECUTED = INVALID_PREDICATE_EXECUTED;
  exports.ACTION_FACTORY_DESC = ACTION_FACTORY_DESC;
  exports.ENTRY_ACTION_FACTORY_DESC = ENTRY_ACTION_FACTORY_DESC;
  exports.UPDATE_STATE_FN_DESC = UPDATE_STATE_FN_DESC;
  exports.PREDICATE_DESC = PREDICATE_DESC;
  exports.COMMAND_RENDER = COMMAND_RENDER;
  exports.CONTRACTS_EVAL = CONTRACTS_EVAL;
  exports.OUTPUTS_MSG = OUTPUTS_MSG;
  exports.INPUT_MSG = INPUT_MSG;
  exports.WARN_MSG = WARN_MSG;
  exports.MACHINE_CREATION_ERROR_MSG = MACHINE_CREATION_ERROR_MSG;
  exports.ERROR_MSG = ERROR_MSG;
  exports.INTERNAL_INPUT_MSG = INTERNAL_INPUT_MSG;
  exports.INTERNAL_OUTPUTS_MSG = INTERNAL_OUTPUTS_MSG;
  exports.DEBUG_MSG = DEBUG_MSG;
  exports.INIT_INPUT_MSG = INIT_INPUT_MSG;
  exports.noop = noop;
  exports.emptyConsole = emptyConsole;
  exports.emptyTracer = emptyTracer;
  exports.isBoolean = isBoolean;
  exports.isFunction = isFunction;
  exports.isControlState = isControlState;
  exports.isEvent = isEvent;
  exports.isActionFactory = isActionFactory;
  exports.make_states = make_states;
  exports.make_events = make_events;
  exports.get_fn_name = get_fn_name;
  exports.wrap = wrap;
  exports.times = times$1;
  exports.always = always;
  exports.keys = keys;
  exports.merge = merge$1;
  exports.is_history_transition = is_history_transition;
  exports.is_entry_transition = is_entry_transition;
  exports.is_from_control_state = is_from_control_state;
  exports.is_to_history_control_state_of = is_to_history_control_state_of;
  exports.is_history_control_state_of = is_history_control_state_of;
  exports.format_transition_label = format_transition_label;
  exports.format_history_transition_state_name = format_history_transition_state_name;
  exports.get_all_transitions = get_all_transitions;
  exports.getDisplayName = getDisplayName;
  exports.mergeModelUpdates = mergeModelUpdates;
  exports.chainModelUpdates = chainModelUpdates;
  exports.mergeActionFactories = mergeActionFactories;
  exports.identity = identity;
  exports.lastOf = lastOf;
  exports.getFsmStateList = getFsmStateList;
  exports.getStatesType = getStatesType;
  exports.getStatesPath = getStatesPath;
  exports.getStatesTransitionsMap = getStatesTransitionsMap;
  exports.getStateEventTransitionsMaps = getStateEventTransitionsMaps;
  exports.getEventTransitionsMaps = getEventTransitionsMaps;
  exports.getHistoryStatesMap = getHistoryStatesMap;
  exports.getTargetStatesMap = getTargetStatesMap;
  exports.getAncestorMap = getAncestorMap;
  exports.computeHistoryMaps = computeHistoryMaps;
  exports.mapOverTransitionsActions = mapOverTransitionsActions;
  exports.reduceTransitions = reduceTransitions;
  exports.everyTransition = everyTransition;
  exports.computeTimesCircledOn = computeTimesCircledOn;
  exports.isInitState = isInitState;
  exports.isInitEvent = isInitEvent;
  exports.isEventless = isEventless;
  exports.arrayizeOutput = arrayizeOutput;
  exports.isHistoryControlState = isHistoryControlState;
  exports.getHistoryParentState = getHistoryParentState;
  exports.isShallowHistory = isShallowHistory;
  exports.isDeepHistory = isDeepHistory;
  exports.getHistoryType = getHistoryType;
  exports.getHistoryUnderlyingState = getHistoryUnderlyingState;
  exports.isHistoryStateEdge = isHistoryStateEdge;
  exports.initHistoryDataStructure = initHistoryDataStructure;
  exports.isCompoundState = isCompoundState;
  exports.isAtomicState = isAtomicState;
  exports.updateHistory = updateHistory;
  exports.computeHistoryState = computeHistoryState;
  exports.findInitTransition = findInitTransition;
  exports.tryCatch = tryCatch;
  exports.tryCatchMachineFn = tryCatchMachineFn;
  exports.getFunctionName = getFunctionName;
  exports.assert = assert;
  exports.notifyThrows = notifyThrows;
  exports.handleFnExecError = handleFnExecError;
  exports.notifyAndRethrow = notifyAndRethrow;
  exports.throwIfInvalidActionResult = throwIfInvalidActionResult;
  exports.throwIfInvalidGuardResult = throwIfInvalidGuardResult;
  exports.throwIfInvalidEntryActionResult = throwIfInvalidEntryActionResult;
  exports.isActions = isActions;
  exports.isEventStruct = isEventStruct;
  exports.isError = isError;
  exports.destructureEvent = destructureEvent;
  exports.formatUndefinedInJSON = formatUndefinedInJSON;
  exports.KinglyError = KinglyError;
